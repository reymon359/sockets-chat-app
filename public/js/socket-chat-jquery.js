...
function renderChatTitle() {
    var html = '';
    html += `<h3 class="box-title">Chat room <small>${params.get('room')}</small> &nbsp;&nbsp;&nbsp; <span> Users: <small>${usersNumber}</small></span> &nbsp;&nbsp;&nbsp;<a href="https://www.addtoany.com/share_save?linkurl=${window.location.hostname}/index.html?room=${params.get('room')}" class="btn btn-link btn-rounded bg-dark text-light" type="submit" target="_blank">Share room</a></h3>`;
    chatTitle.html(html);
}
